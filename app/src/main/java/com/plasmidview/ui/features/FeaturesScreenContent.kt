...
fun FeaturesScreenContent(docIndex: Int) {
    ...
    Column(Modifier.fillMaxSize()) {
        // Plasmid AI loading at top
        if (plasmidAiLoading) { LinearProgressIndicator(Modifier.fillMaxWidth()) }
        ...
        Surface(color = MaterialTheme.colorScheme.surfaceVariant.copy(alpha = 0.5f), modifier = Modifier.fillMaxWidth()) {
            Row(Modifier.padding(horizontal = 16.dp, vertical = 8.dp), verticalAlignment = Alignment.CenterVertically) {
                ...
                Box {
                    IconButton(onClick = {
                        scope.launch {
                            plasmidAiLoading = true; plasmidAiResult = null
                            ...
                            plasmidAiResult = client.ask("You are a molecular biology expert.", prompt)
    ...
    if (plasmidAiResult != null) {
        AlertDialog(
            onDismissRequest = { plasmidAiResult = null },
            title = { Text("About this plasmid", fontWeight = FontWeight.Bold) },
            text = {
                Column(Modifier.verticalScroll(rememberScrollState()).fillMaxWidth()) {
                    MarkdownText(markdown = plasmidAiResult!!.ifBlank { "No response from AI." }, style = MaterialTheme.typography.bodyMedium)
                }
            },
            confirmButton = {
                    ...
                    Button(onClick = {
                        if (aiResult == null && !aiLoading) {
                            scope.launch {
                                ...
                                aiResult = client.ask("You are a molecular biology expert specializing in plasmid design.", prompt, fast = true)
                                aiLoading = false
                            }
                        }
                    }) {
